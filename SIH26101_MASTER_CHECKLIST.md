...
- [x] ~~**Keep Quest mode as an optional engagement layer.**~~ **SUPERSEDED 6 September 2026:** the team has since decided to scrap Quest mode's RPG framing rather than keep it as a secondary mode -- confirmed directly by Shashwat (Lane 2 lead) in-session. `/stats` and `/guild` were rebuilt on the professional shell (real data, no `Pixel*`/dark-cave styling) as the first step; `/dungeon`, `/combat`, `/boss`, `/character` still use the RPG skin and are the remaining cleanup, not yet scheduled. Original text, kept for history: "Do not execute the pasted recommendation to delete it. The primary experience must become a professional learning workspace; Quest mode is a secondary practice mode... Quest's pixel-art/dungeon skin visually scoped to opt-in Quest routes (`/dungeon`, `/combat`, `/boss`, `/guild`, `/character`) only." **DECISION (REVERSED)**
