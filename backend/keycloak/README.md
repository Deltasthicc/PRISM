...
- Admin console: <http://localhost:8180> (`admin` / `sih_dev_local_only`)
- Realm: `sih-learning-tool` (auto-imported from `sih-realm-export.json` on
...
- Token endpoint: `http://localhost:8180/realms/sih-learning-tool/protocol/openid-connect/token`
- Discovery document: `http://localhost:8180/realms/sih-learning-tool/.well-known/openid-configuration`

## Test users

Every user's password is `sih_dev_local_only`. Each has exactly one realm
...
```powershell
curl -X POST http://localhost:8180/realms/sih-learning-tool/protocol/openid-connect/token `
  -H "Content-Type: application/x-www-form-urlencoded" `
  -d "grant_type=password" `
  -d "client_id=sih-backend-dev" `
  -d "client_secret=sih_dev_local_only_client_secret" `
  -d "username=demo-learner" `
  -d "password=sih_dev_local_only" `
  -d "scope=openid"
```

The `sih-backend-dev` client uses the OAuth2 Resource Owner Password
...
The access token includes an explicit `aud: "sih-backend-dev"` claim (via
the client's `sih-backend-audience` protocol mapper) — Keycloak does not add
