...
// One Learner Assistant, two ways to reach the exact same real,
// access-filtered RAG engine (backend/ai/retrieval.py + ai/assistant.py) --
// type a question, or hold a live voice conversation with it
// (backend/routes/ai_voice.py's WebSocket protocol). These were two
// separate NavBar tabs and two separate pages; merged into one page with a
// side-by-side mode toggle (matching /quiz's existing upload/fixed/library
// tab pattern) since they're really one feature with two input methods, not
...
function VoiceAssistantPanel({ t }) {
    ...
    <div className="max-w-3xl mx-auto flex flex-col gap-4">
      ...
      <div className="flex items-center gap-1 bg-[#f2f3ff] rounded-lg p-1 self-start">
        <button
          ...
          className={`font-mono text-[11px] uppercase tracking-wider px-3 py-2 rounded-md flex items-center gap-1.5 ${mode === 'text' ? 'bg-white text-[#00236f] shadow-sm' : 'text-[#757682]'}`}
        >
        ...
        <button
          ...
          className={`font-mono text-[11px] uppercase tracking-wider px-3 py-2 rounded-md flex items-center gap-1.5 ${mode === 'voice' ? 'bg-white text-[#00236f] shadow-sm' : 'text-[#757682]'}`}
        >
      ...
      {mode === 'text' ? <TextAssistantPanel t={t} /> : <VoiceAssistantPanel t={t} />}
    </div>
  );
}
