...
const CompetencyGapView = ({
  ...
  const currentBenchmark = benchmarks.find(b => b.id === selectedBenchmarkId) || benchmarks[0];
  const selectedDimension = dimensions.find(d => d.id === selectedDimId) || dimensions[0];
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="flex items-center justify-between py-1.5 px-3 md:px-4 bg-[#f2f3ff] rounded-lg mb-4 border border-[#c5c5d3]/30 text-xs">
        ...
        <div className="flex items-center gap-1.5 text-xs text-[#444651]">
          <span className="material-symbols-outlined text-[#757682] text-[16px]">verified</span>
          <span className="font-mono text-xs">Attested by Cadre Cell (CSO-HQ)</span>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 bg-[#f2f3ff] px-3 py-1.5 rounded-lg border border-[#c5c5d3]/30">
              <span className="font-mono text-xs text-[#757682]">Target:</span>
              <select
              ...
              >
                {benchmarks.map((b) => (
                  <option key={b.id} value={b.id} className="text-[#131b2e]">
                    {b.title}
                  </option>
                ))}
              </select>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-5 items-start">
        {/* Left Column: Vector Projection Radar & Benchmark Breakdown (Col 5) */}
        ...
        {/* Right Column: Streamlined Domain Gap Vector Cards (Col 7) */}
